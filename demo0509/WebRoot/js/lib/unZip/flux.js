...
(function($){
	flux.slider = function(elem, opts) {
		...
		// Listen for click events as we may want to follow a link  // 修改绑定事件类型click为tap
		this.container.bind('tap', function(event) {
	...
	flux.slider.prototype = {
		...
		finishedLoading: function() {
			...
			{
				this.bullets = $('<ul class="bullets"></ul>').css($.extend({
					position : 'absolute' ,
					bottom : '0.8em',right:'12px',
					padding: '0px',
					'overflow' : 'hidden',
					'line-height' : 0,
					'z-index' : 111,
			...
			// Should we use captions? //添加了captionsStyle
			if(this.options.captions)
			{
				this.captionBar = $('<div class="caption"></div>').css($.extend({
					...
					width: '100%',
					bottom: 0 
				},this.options.captionsStyle||{})).css3({
		...
		return new flux.transition_grid(fluxslider, $.extend({
			...
			renderTile: function(elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
				var bar = $('<div class="bar-'+colIndex+'"></div>').css({
					...
					'background-image': this.slider.image1.css('background-image'),
					'background-size' : this.slider.image1.width()*100/colWidth+'% '+this.slider.image1.height()*100/rowHeight+'%',
					'background-position': '-'+leftOffset+'px 0px',
			...
			setup: function() {
				...
				for(var i=0; i<blockCount; i++)
				{
					...
					var block = $('<div></div>').attr('class', 'block block-'+i).css({
						...
						'background-image': this.slider.image1.css('background-image'),
						'background-size' : w*100/thisBlockSize+'% '+h*100/thisBlockSize+'%' ,
